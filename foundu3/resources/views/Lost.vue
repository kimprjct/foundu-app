<template>
  <div class="main-container">
    ...
    <div class="top-section">
      ...
      <router-link to="/report-lost" class="report-button">
        <span>Report</span>
        <img :src="lostIcon" alt="Report Icon" />
      </router-link>
    </div>
    <div class="lost-items-grid">
      <div
        ...
        :key="index"
        class="lost-item-column"
      >
        <div class="reporter-info">
          <div class="profile-circle">{{ reporterFullName.charAt(0) }}</div>
          <div class="reporter-name">{{ reporterFullName }}</div>
        </div>
        <img :src="item.photoURL" alt="Lost Item" class="lost-item-image" />
        <div class="item-details">
          <div class="lost-item-name">{{ item.itemName }}</div>
          <div class="lost-item-location">{{ item.landmark }}</div>
          <div class="lost-item-date">{{ item.dateLost }} | {{ item.timeLost }}</div>
        </div>
        <p class="lost-item-description">{{ item.itemDescription }}</p>
        <div class="found-it-container">
          <div
            class="found-it-button"
            :class="{ 'found-item-under-review': item.found }"
            :style="{ backgroundColor: item.found ? '#11A104' : '#65558F' }"
            @click="markItemAsFound(index)"
          >
            {{ item.found ? "Found Item Under Review" : "Found It" }}
          </div>
        </div>
      </div>
    </div>
...
.report-button {
  ...
  background: linear-gradient(90deg, rgba(153, 19, 19, 0.72) 0%, rgba(255, 31, 31, 0.72) 74.5%);
...
.lost-item-location, .lost-item-date {
    font-size: 12px;
    margin-bottom: 5px;
    text-align: left;
}
